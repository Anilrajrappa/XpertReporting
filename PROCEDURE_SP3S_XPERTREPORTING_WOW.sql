...
LBL1:
	...
	when  a.XPERT_REP_CODE='R3' then 'Transaction Summary'
	when  a.XPERT_REP_CODE='R4' then 'Pendency Analysis'	
	when  a.XPERT_REP_CODE='R5' then 'Customer Analysis'	
	...
	AND ISNULL(a.XPERT_REP_CODE ,'') NOT IN ('', 'R6','XNHISTORY')
	...
	WHERE   r.user_code = @cUserCode
	And isnull(a.XPERT_REP_CODE,'') <> ''   AND isnull(a.xn_history,0)<> 1

	UNION 
	 SELECT 'Z' AS XN_TYPE,a.rep_id,a.rep_name,
	case when a.XPERT_REP_CODE='R2' then 'Transaction Analysis'  
	when  a.XPERT_REP_CODE='R3' then 'Transaction Summary'
	when  a.XPERT_REP_CODE='R4' then 'Pendency Analysis'	
	when  a.XPERT_REP_CODE='R5' then 'Customer Analysis'	
	Else  'Stock Analysis' End  as rep_type,0 as  inActive,
	isnull(a.user_rep_type,'ALL') as user_rep_type,a.remarks,c.username ,a.Last_update,a.XPERT_REP_CODE
	FROM rep_mst a (NOLOCK) 	
	JOIN USERS C on a.user_code = c.user_code 
	WHERE  isnull(a.report_item_type,1) = 1  AND isnull(a.xn_history,0)<> 1
	AND ISNULL(a.XPERT_REP_CODE ,'')  IN ('R1', 'R2')
    and a.user_code =@cUserCode
...
LBL3:
	...
	UNION ALL
	Select 'R3' as report_grp_code ,'Transaction Summary' as report_group 
	UNION ALL
	Select 'R4' as report_grp_code ,'Pendency Analysis' as report_group 
	UNION ALL
	Select 'R5' as report_grp_code ,'Customer Analysis' as report_group 
